...
//        List<Atlas> allAtlases = atlasRepository.findAll(); // 모든 지역별 도감 원본
        List<Atlas> atlases;
        if (areaGroup != null) { // areaGroup 파라미터가 있으면, 해당 지역만 조회
            atlases = List.of(atlasRepository.findByAreaGroup(areaGroup)
                    .orElseThrow(() -> new AtlasNotFoundException()));
        } else { // areaGroup 파라미터가 없으면, 모든 지역 조회
            atlases = atlasRepository.findAll();
        }

        List<UserAtlas> userAtlases = userAtlasRepository.findByUser(user); // 유저의 모든 지역별 도감
