...
//    List<AtlasReward> findByAtlasIdInOrderByTargetLevel(@Param("atlasIds") List<Long> atlasIds);
    ...
    @Query("SELECT ar FROM AtlasReward ar JOIN FETCH ar.atlas WHERE ar.atlas.id IN :atlasIds ORDER BY ar.atlas.id, ar.targetLevel")
    List<AtlasReward> findByAtlasIdInOrderByTargetLevelWithAtlas(@Param("atlasIds") List<Long> atlasIds); // atlas를 함께 조회
}
